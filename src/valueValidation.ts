...
const errors:{[key:string]:string} = {
  ...
  undefined: "Undefined validation type, if its used by DD and is missing, contact Zot, otherwise doubled check if its the correct value."
}
...
function validateSimple(value:string, regex:RegExp, error:string){
    ...
    if(valid != true || valid == null) return {isValid: false, message: error};
    return {isValid: true, message: null};
}

function validateRange(range:string){
    const items = range.split(/\s+/)
    if (items.length != 2) return {isValid: false, message: errors['range']};
    for (let item of items) {
        const result = validateSimple(item, regexTypes['number'], errors['number']);
        if (!result.isValid) {
            return {isValid: false, message: `Range parameter "${item}" is invalid. ${errors['number']}`,};
        }
    }
    return { isValid: true, message: null };
}
...
function validateList(list:string, listType:string){
    ...
    for (let item of items) {
        ...
        if (!result.isValid) {
            return {isValid: false, message: `Item "${item}" is invalid. ${errors[listType]}`,};
        }
    }
    return { isValid: true, message: null };
}